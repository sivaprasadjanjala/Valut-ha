...
module VaultCookbook
 module Resource
   ...
   class VaultConfig < Chef::Resource
     ...
     attribute(:backend_type, default: 'dynamodb', equal_to: %w(consul etcd zookeeper dynamodb s3 mysql postgresql inmem file))
     ...
     attribute(:max_parallel, kind_of: Integer, default: 128)
     #attribute(:recovery_mode, kind_of: Integer, default: 1)
     attribute(:ha_enabled, equal_to: [true, false], default: true)
     ...
     def to_json
       # top-level
       config_keeps = %i(cache_size disable_cache disable_mlock ui default_lease_ttl max_lease_ttl api_addr)
       ...
       # backend
       backend_keeps = %i(region table)
       backend_options = to_hash.keep_if do |k, _|
         backend_keeps.include?(k.to_sym)
       end.merge(max_parallel: max_parallel, ha_enabled: ha_enabled.to_s)
       config['storage'] = { backend_type => backend_options }
